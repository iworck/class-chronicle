-- Update trigger: only set APROVADO/REPROVADO when ALL template grades are entered
-- If any grade is missing, keep CURSANDO
...
DECLARE
  ...
  v_total_template_items INT;
  v_total_grades_entered INT;
  v_all_grades_entered BOOLEAN := false;
BEGIN
  ...
  IF v_has_template THEN
    -- Count total leaf template items (the ones that need grades)
    SELECT COUNT(*) INTO v_total_template_items
    FROM grade_template_items
    WHERE class_subject_id = v_cs_id
      AND (
        -- Items with parent (children/leaves)
        parent_item_id IS NOT NULL
        -- OR parent items with no children (standalone)
        OR (counts_in_final = true AND NOT EXISTS(
          SELECT 1 FROM grade_template_items child
          WHERE child.parent_item_id = grade_template_items.id AND child.class_subject_id = v_cs_id
        ))
      );

    -- Count how many of those have grades entered
    SELECT COUNT(*) INTO v_total_grades_entered
    FROM grade_template_items gti
    JOIN student_grades sg ON sg.enrollment_id = v_enrollment_id
      AND sg.grade_type = gti.name
    WHERE gti.class_subject_id = v_cs_id
      AND (
        gti.parent_item_id IS NOT NULL
        OR (gti.counts_in_final = true AND NOT EXISTS(
          SELECT 1 FROM grade_template_items child
          WHERE child.parent_item_id = gti.id AND child.class_subject_id = v_cs_id
        ))
      );

    v_all_grades_entered := (v_total_template_items > 0 AND v_total_grades_entered >= v_total_template_items);

    -- If not all grades entered, ensure status is CURSANDO
    IF NOT v_all_grades_entered THEN
      UPDATE student_subject_enrollments SET status = 'CURSANDO', updated_at = now() WHERE id = v_enrollment_id AND status != 'TRANCADO';
      RETURN COALESCE(NEW, OLD);
    END IF;

    -- All grades entered: calculate the final average
  ...
  ELSE
    -- No template: fallback weighted average
