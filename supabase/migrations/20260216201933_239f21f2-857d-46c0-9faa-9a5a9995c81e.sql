...
-- Update trigger function to respect grades_closed flag
...
BEGIN
  ...
  SELECT cs.id, cs.grades_closed INTO v_cs_id, v_grades_closed
  ...
  -- If grades are not closed, always keep CURSANDO
  IF NOT v_grades_closed THEN
    UPDATE student_subject_enrollments SET status = 'CURSANDO', updated_at = now() WHERE id = v_enrollment_id AND status NOT IN ('TRANCADO');
